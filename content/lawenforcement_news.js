...
const sourceUrl = 'https://lawenforcement.numberportability.com/news/'
...
const writeToCsv = async (dataArr) => {
	...
	// Save to file:
	await csv.toDisk('./content/lawenforcement_news.csv');
...
async function fetchNewsData() {
	...
	const promises = $articlesArr.map(async (index, item) => {
		...
		const sourceIsInternal = articleData.url.indexOf('lawenforcement.numberportability.com') > 0
